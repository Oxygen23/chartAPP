...
$role_stmt = $conn->prepare("SELECT role FROM user_room_roles WHERE user_id = ? AND room_name = ?");
$role_stmt->execute([$current_user_id, $room]);
$room_user_role = $role_stmt->fetchColumn() ?: "member";

if (isset($_POST["send_message"])) {
    $message = trim($_POST["message"]);
    $user_id = $current_user_id;

    if ($room_user_role === "banned") {
        ...
        $notify = $conn->prepare(
            "INSERT INTO notifications (user_id, message_id, room_name) VALUES (?, ?, ?)"
        );
    ...
    <main class="chat-page">
        <section class="chat-box">
            <div class="messages" id="messagesBox">
                ...
                <?php if ($room_user_role !== "member") { ?>
                    <div class="room-role-banner">
                        Role in this room: <strong><?php echo htmlspecialchars(ucfirst($room_user_role)); ?></strong>
                    </div>
                <?php } ?>
            ...
            <form method="POST" class="message-form" id="messageForm">
                <textarea name="message" id="messageInput" placeholder="Write a message..." <?php echo $room_user_role === "banned" ? "disabled" : "required"; ?>></textarea>
                <small class="field-error" id="messageError"></small>
                <button type="submit" name="send_message" <?php echo $room_user_role === "banned" ? "disabled" : ""; ?>>Send</button>
                <?php if ($room_user_role === "banned") { ?>
                    <p class="field-error">You are banned from posting in this room.</p>
                <?php } ?>
            </form>
        </section>
    </main>
